<script lang="ts">
    ...
    // Get topic and article ID from URL params
    ...
    async function loadArticle() {
        if (!currentTopic || !articleId) {
            error = 'Invalid article URL. Missing topic or article ID.';
            loading = false;
            return;
        }
        ...
        try {
            ...
            article = await getAnalystArticle(currentTopic, articleId);
    ...
    async function handleArticleSubmittedAction(action: UIAction): Promise<ActionResult> {
        ...
        await goto(currentTopic ? `/analyst/${currentTopic}` : '/analyst');
        return { success: true, action: 'article_submitted', message: 'Article submitted, returning to hub' };
    }

    // Load article when topic or ID changes
    $: if (currentTopic && articleId && $auth.isAuthenticated) {
        loadArticle();
    }
...
<div class="editor-container">
    ...
    {:else if error && !article}
        <div class="error-screen">
            <h2>Error</h2>
            <p>{error}</p>
            <button on:click={() => goto(currentTopic ? `/analyst/${currentTopic}` : '/')}>
                {currentTopic ? `Back to ${currentTopic.toUpperCase()}` : 'Back to Home'}
            </button>
        </div>
    {:else if article}
