...
IMPORTANT: Do NOT ask any questions. Do NOT request clarification. Just write the complete article based on the information provided."""
        ...
        keywords_response = llm.invoke([
            ...
            HumanMessage(content=f"Article headline: {headline}\n\nContent excerpt: {content_response.content[:1500]}\n\nKeywords:"),
        ])
        keywords = keywords_response.content.strip()

        logger.info(f"✍️ Generated content: headline='{headline}', keywords='{keywords}', ~{len(content_response.content)} chars")
