"""
Editor node for the main chat graph.

This node handles editor role context:
- Editorial workflow (review, publish, reject)
- Editor UI actions
- Managing the editorial queue
- HITL for publish confirmations

Routes based on nav_context.role == 'editor'
"""
...
from agents.shared.permission_utils import check_topic_permission, get_topics_for_role, validate_article_access
...
EDITOR_UI_ACTIONS = {
    ...
    "publish_article": {"requires_article": True, "workflow": True},
    "reject_article": {"requires_article": True, "workflow": True},
    "review_article": {"requires_article": True, "workflow": True},
    # Downloads
...
def editor_node(state: AgentState) -> Dict[str, Any]:
    """
    Handle editor role context requests.

    This node handles:
    1. Editor UI actions (view, download, select)
    2. Editorial workflow (review, publish, reject)
    3. Queue management (list pending)
    ...
    if action_type in EDITOR_UI_ACTIONS:
        action_config = EDITOR_UI_ACTIONS[action_type]
        if action_config.get("workflow"):
            # Map UI action to workflow action
            action_map = {
                "publish_article": "publish",
                "reject_article": "reject",
                "review_article": "review",
            }
            details["action"] = action_map.get(action_type, "review")
        else:
            # Handle non-workflow UI actions
            return _handle_editor_ui_action(action_type, details, user_context, nav_context)

    logger.info(f"editor_node: ENTRY - intent={intent}, nav_context={nav_context}")
...
def _handle_editor_ui_action(
    action_type: str,
    details: Dict[str, Any],
    user_context: Dict[str, Any],
    nav_context: Dict[str, Any]
) -> Dict[str, Any]:
    """Handle non-workflow editor UI actions."""
    topic = details.get("topic") or nav_context.get("topic")
    article_id = details.get("article_id") or nav_context.get("article_id")

    # Validate article access if needed
    action_config = EDITOR_UI_ACTIONS.get(action_type, {})
    if action_config.get("requires_article") and article_id:
        try:
    ...
    response_text = responses.get(action_type, f"Executing {action_type}...")
    ...
    return {
        ...
        "selected_agent": "editor",
        "routing_reason": f"Editor action: {action_type}",
        "is_final": True
    }
...
def _handle_list_pending(topic: Optional[str], user_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    try:
        from agents.shared.editor_sub_agent import EditorSubAgent
        from database import SessionLocal
        ...
        try:
            ...
            articles_list = "\n".join([
                f"- **#{a.get('article_id', '')}**: {a.get('article_headline', '')} ({a.get('topic', '')}) - by {a.get('requested_by', '')}"
                for a in pending[:10]
            ])
            ...
            return {
                ...
                "referenced_articles": [
                    {"id": a.get("article_id"), "headline": a.get("article_headline", ""), "topic": a.get("topic", "")}
                    for a in pending
                ],
                "navigation": {
                    "action": "navigate",
