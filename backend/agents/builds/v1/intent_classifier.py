...
from agents.builds.v1.state import IntentClassification, IntentType, NavigationContext
...
1. **ui_action** - User wants to navigate to a page OR trigger a UI action
   ...
   **Page Navigation** (use these action values):
   ...
   - goto_reader_topic: "read macro articles", "view equity content"
   - goto_analyst_topic: "go to analyst hub", "analyst dashboard", "open analyst section"
   - goto_editor_topic: "go to editor hub", "editor dashboard", "open editor section", "go to editor"
   - goto_admin_topic: "go to admin", "take me to {{topic}} admin", "admin panel", "admin dashboard"
   - goto_root: "global admin", "manage topics", "manage users", "system settings"
   - goto_user_profile: "go to profile", "my profile", "view my profile"
   - goto_user_settings: "account settings", "my settings", "user settings"
   - edit_article: "edit article #9", "open article 15 in editor" - MUST extract article_id from message

   **Admin Navigation Notes**:
   - "take me to macro admin" → goto_admin_topic with topic="macro"
   - "go to admin" → goto_admin_topic (uses current topic from context)
   - "global admin" or "manage topics" → goto_root
   ...
   **View Switching** (within analyst page only):
   ...
   IMPORTANT: "editor dashboard" or "editor hub" means goto_editor_topic (the editor PAGE).
...
def _infer_navigation_action(message: str, nav_context: Dict[str, Any]) -> Optional[str]:
    ...
    # "editor" alone without "view" should also be goto_editor_topic
